    ...
    <View className="mb-6">
      <View className="mb-5 flex-row items-start justify-between">
        ...
        <View className="items-center rounded-[16px] bg-deck-card px-4 py-3">
          <AppText
            className="text-[24px] leading-[27px] text-white"
            weight="black"
          >
            {totalCards}
          </AppText>
      ...
      <View className="flex-row justify-around rounded-[20px] bg-deck-card px-5 py-4">
        <Score color={colors.green} label="Acertos" value={`${goodCount}`} />
        <Score
          color={colors.orange}
          label="Restantes"
          value={`${Math.max(totalCards - goodCount - againCount, 0)}`}
        />
        <Score color={colors.red} label="Errados" value={`${againCount}`} />
      </View>
    </View>
  );
}
